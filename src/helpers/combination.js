export const Combination = (function () {
  function Combination(n, k) {
    if (n && k) {
      if (n < 0 || k < 0) throw new Error('Negative parameter in constructor')
      this.n = n
      this.k = k
      this.data = []
      for (var i = 0; i < k; i++) {
  ...
  Combination.prototype.successor = function () {
    if (this.data.length === 0 || this.data[0] === this.n - this.k) return null
    var result = new Combination(this.n, this.k)
    var i = 0
    for (i = 0; i < this.k; i++) result.data[i] = this.data[i]
    for (i = this.k - 1; i > 0 && result.data[i] === this.n - this.k + i; ) i--
    result.data[i]++
    for (var j = i; j < this.k - 1; j++) result.data[j + 1] = result.data[j] + 1
    return result
  }
  Combination.prototype.first = function () {
    var result = new Combination(this.n, this.k)
    for (var i = 0; i < result.k; ++i) result.data[i] = i
    return result
  }
...
export const FastCombination = (function () {
  ...
  FastCombination.prototype.chooseFromSets = function (setSizes, k) {
    ...
    for (var y = 1; y <= n - k + 1; y++) {
      if (y === 1) {
        for (var x = 1; x <= k; x++) {
          m[x][y] = setSizes.take(x).reduce(function (product, next) {
            return product * next
          }, 1)
          result = m[x][y]
        }
      } else {
        m[1][y] = setSizes.take(y).reduce(function (a, b) {
          return a + b
        }, 0)
        result = m[1][y]
        for (var x = 2; x <= k; x++) {
