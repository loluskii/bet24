...
import CalculatedCoupon from './calculated-coupon'
...
export default class CouponCalculation {
  ...
  getOddsForSlotKeyMap = function (coupon) {
    ...
    for (var i = 0; i < coupon.selections.length; i++) {
      ...
      for (var key in oddsForSlotKeyMap) {
        ...
        if (slotKey.MatchId === odd.matchId && slotKey.Index === 0) keys.push(slotKey)
        else if (slotKey.MatchId === odd.matchId && slotKey.Index > 0)
          throw new Error('Mixed CompatibilityLevel on odds')
      }
      if (keys.length > 1) throw new Error('Wrong oddsForSlotKeyMap construction')
      if (keys.length === 0) {
  ...
  calcCombinations = function (coupon) {
    // console.log('calculating combinations', coupon);
    var maxCombinationForCoupon = Math.min(
      import.meta.env.VITE_MaxCombinationsByGrouping,
      import.meta.env.VITE_MaxCouponCombinations
    )
    if (
    ...
    if (!integrale) {
      ...
      for (var k = nonBankers.length; k > 0; k--) {
        var combinationsCount = this.calcCombinationsForCrossCombinationsBetGroup(
          k,
  ...
  bonusPercentageFromNumberOfEvents = function (bonusList, numberOfEvents) {
    console.log('min/max bonus', bonusList.min('ticket_length'),  bonusList.max('ticket_length'))
      console.log('no of events', numberOfEvents)
    try {
      ...
      const filteredBonus = bonusList.find(function (b) {
        console.log(b.ticket_length, numberOfEvents)
        return b.ticket_length == numberOfEvents
      });
      console.log('filtered bonus', filteredBonus);

      if (filteredBonus) {
        return filteredBonus.bonus / 100.0
      }
      return 0.0
    } catch(e) {
      console.log("Can't find the bonus percentage for " + numberOfEvents + ' events', e.message);
      return 0.0
    }
  }
  updateFromCalculatedCoupon = function (coupon, calculatedCoupon, globalVar, bonusList) {
  ...
  setPossibleCouponStake = function (betCoupon) {
    for (var _d = 0, _e = betCoupon.Groupings; _d < _e.length; _d++) {
      ...
      var minInnerStakeAllowed = import.meta.env.VITE_MinGroupingsBetStake
      if (betCoupon.stake !== 0) {
        ...
        minInnerStakeAllowed = Math.max(
          import.meta.env.VITE_MinGroupingsBetStake,
          import.meta.env.VITE_MinBetStake * grpStakeRatio
        )
      }
      if (
        import.meta.env.VITE_StakeInnerMod0Combination != null &&
        import.meta.env.VITE_StakeInnerMod0Combination !== 0
      ) {
        var modValueItem = grpItem.Stake % import.meta.env.VITE_StakeInnerMod0Combination
        //if correct then continue to next GroupItem
        if (modValueItem !== 0) {
          //new Stake Value
          var divValueItem = grpItem.Stake / import.meta.env.VITE_StakeInnerMod0Combination
          var rounded = Math.round(divValueItem)
          if (rounded === 0) rounded = 1
          grpItem.Stake = rounded * import.meta.env.VITE_StakeInnerMod0Combination
        }
        if (grpItem.Stake < minInnerStakeAllowed) {
          //new Stake Value
          var divValueItemMin =
            minInnerStakeAllowed / import.meta.env.VITE_StakeInnerMod0Combination
          var roundedMin = Math.ceil(divValueItemMin)
          if (roundedMin === 0) roundedMin = 1
          grpItem.Stake = roundedMin * import.meta.env.VITE_StakeInnerMod0Combination
        }
      }
  ...
  setPossibleGroupings = (betCoupon, calculatedCoupon) => {
    ...
    for (var i = 0; i < calculatedCoupon.Groups.length; i++) {
      ...
      if (betCouponGroups.length > 1) throw new Error('Multiple Groupings defined!')
      //add all groupings
    ...
    if (betCoupon.Groupings.length === 0 && possibleMissingGrouping.length >= 1) {
      ...
      var validGroupings = possibleMissingGrouping.filter(function (g) {
        return g.Grouping > 0 && g.Grouping !== 1 && g.Grouping !== betCoupon.Odds.length
      })
      ...
      if (lastGrouping) {
        ...
        possibleMissingGrouping = possibleMissingGrouping.filter(function (g) {
          return (
            g.Grouping !== lastGrouping.Grouping || g.Combinations !== lastGrouping.Combinations
          )
        })
      }
    }
