const CalculatedCoupon = (function () {
  ...
  CalculatedCoupon.prototype.getMaxAllowedStake = function (
    lossThreshold,
    stakeThreshold,
    maxGrouping,
    selectedGroupings,
    totalCouponCombinations
  ) {
    var totalCurrentLossForUnit =
      this.getTotalWinForUnit(selectedGroupings) + this.getTotalBonusForUnit(selectedGroupings)
  ...
  CalculatedCoupon.prototype.getTotalWinForUnit = function (selectedGroupings) {
    if (selectedGroupings.length > 0) {
      return this.Groups.filter(function (g) {
        return selectedGroupings.indexOf(g.Grouping) !== -1
      }).sum('MaxWinForUnit')
    } else {
      return 0
    }
