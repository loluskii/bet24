  ...
  <div class="m-trans-wrap">
    <div id="transaction-list-container">
      ...
      <template v-if="viewMode === 'list'">
        <div class="m-custom-selection-wrapper">
          <div class="m-calendar m-custom-date-active">
            <VueDatePicker
              v-model="range"
              range
        ...
        <div class="list-bg" v-else style="position: relative">
          <ul v-if="transactionData" class="m-trans-list">
            ...
            >
              <div class="m-trans-r1">
                ...
                <i
                  data-op="transaction_amount"
                  class=""
                  :class="item.type == 'credit' && item.status ? 'm-txt-green' : 'm-txt-dark-gray'"
                >
                  {{ item.type == 'credit' && item.status ? '+' : item.type == 'debit' ? '-' : '' }}
                  {{ formatNumber(item.amount) }}
                </i>
              </div>
              <div class="m-trans-r2">
                <em>{{ formatDate(item.transactionDate, 'YYYY-MM-DD h:mma') }}</em>
                <i class="text-danger" v-if="item.status === 4">Cancelled</i>
                <i class="text-danger" v-if="item.status === 3">Rejected</i>
                <i class="text-danger" v-if="item.status === 2">Failed</i>
                <i class="text-success" v-if="item.status === 1">Successful</i>
          ...
          <div v-else class="m-list-none">
            ...
            >
            <div
              class="no-list-more-records"
              data-cms-key="select_a_date_range_to_find_more_records"
              data-cms-page="page_transaction"
            >
              Select a date range to find more records
            </div>
          </div>
        </div>
      </template>
      <template v-else>
        <TransactionDetail :data="selectedTransaction" :type="'transactions'" />
      </template>
    </div>
  </div>
</template>
...
import { getTransactionsData, formatDate, formatNumber } from '@/helpers'
...
async function getData(start, end) {
  try {
    ...
    const res = await getTransactionsData(payload, 1)
